...
## BLOB handling - attachments
When binary data (images, documents, media) needs to be associated with a document, RavenDB provides the `Attachment API`. 
...
```
## Queries
RavenDB uses indexes to retrieve data but they don't work the same way as relational database indexes. The main difference is that RavenDB's indexes are schema-less documented oriented. 
RavenDB requires an index to solve a query;that is why indexe is needed. The great think is that a programmer is not required to manually create indexes - RavenDB can deduct and create 
